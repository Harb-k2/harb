...
import { Code2, Eye, FileCode2, Files } from "lucide-react";
import { useEffect, useMemo, useState } from "react";

export type ProjectPreviewFile = { path: string; content: string; truncated: boolean; lineCount: number };

export function ProjectCodePreview({ files }: { files: ProjectPreviewFile[] }) {
    ...
    <section className="harb-project-preview mt-5 overflow-hidden rounded-2xl border border-white/10 bg-[#0b131e]/80" aria-label="محرر معاينة المشروع">
      ...
      <div className="grid min-h-[300px] lg:grid-cols-[210px_minmax(0,1fr)]">
        <aside className="border-b border-white/10 bg-black/15 p-2 lg:border-b-0 lg:border-l" aria-label="ملفات المشروع">
          ...
          <div className="max-h-44 space-y-1 overflow-y-auto lg:max-h-80">{files.map(file => <button key={file.path} type="button" onClick={() => setSelectedPath(file.path)} className={cn("flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-right text-xs transition-colors", selected.path === file.path ? "bg-primary/12 text-foreground" : "text-muted-foreground hover:bg-white/5 hover:text-foreground")}><FileCode2 className="h-3.5 w-3.5 shrink-0 text-primary" /><span className="truncate" dir="ltr">{file.path}</span></button>)}</div>
        </aside>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 px-4 py-2.5"><p className="truncate font-mono text-xs text-primary" dir="ltr">{selected.path}</p><span className="text-[10px] text-muted-foreground">{selected.lineCount} سطر</span></div>
