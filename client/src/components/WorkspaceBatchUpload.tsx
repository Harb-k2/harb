...
import { Archive, CheckCircle2, Code2, FileCode2, FileText, FolderArchive, Image, Loader2, Search, UploadCloud, XCircle } from "lucide-react";
...
type ArchiveResult = { path: string; match: "name" | "content"; line?: number; snippet?: string };
...
export function WorkspaceBatchUpload() {
  ...
  const [archiveQueries, setArchiveQueries] = useState<Record<string, string>>({});
  const [archiveResults, setArchiveResults] = useState<Record<string, ArchiveResult[] | undefined>>({});
  const upload = trpc.harb.files.upload.useMutation();
  const inspect = trpc.harb.files.inspect.useMutation();
  const searchArchive = trpc.harb.files.searchArchive.useMutation();
  ...
  const runArchiveSearch = (item: QueueItem) => {
    const query = archiveQueries[item.id]?.trim();
    if (!item.artifact || !query || query.length < 2) return toast.info("اكتب عبارتين على الأقل للبحث داخل الحزمة.");
    searchArchive.mutate({ id: item.artifact.id, query }, {
      onSuccess: result => setArchiveResults(current => ({ ...current, [item.id]: result.results })),
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-black/10 p-4">
      ...
      {items.length > 0 && <div className="mt-4 space-y-3">{items.map(item => <article key={item.id} className="rounded-xl border border-white/10 bg-background/30 p-3"><div className="flex items-start gap-3"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/5">{item.inspection?.kind === "archive" ? <FolderArchive className="h-4 w-4 text-primary" /> : item.inspection?.kind === "code" ? <Code2 className="h-4 w-4 text-primary" /> : item.inspection?.kind === "image" ? <Image className="h-4 w-4 text-primary" /> : <FileCode2 className="h-4 w-4 text-primary" />}</span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-medium">{item.file.name}</p><span className="flex shrink-0 items-center gap-1 text-[11px] text-muted-foreground">{statusIcon(item.status)}{statusLabel(item.status)}</span></div>{item.inspection && <p className="mt-1 text-xs leading-5 text-muted-foreground">{item.inspection.summary}</p>}{item.error && <p className="mt-1 text-xs leading-5 text-rose-200">{item.error}</p>}</div></div>{item.inspection?.archiveFiles?.length ? <div className="mt-3 rounded-lg border border-white/8 bg-black/15 p-2"><p className="text-[11px] font-semibold text-muted-foreground">محتويات الحزمة الآمنة</p><div className="mt-1.5 flex flex-wrap gap-1.5">{item.inspection.archiveFiles.slice(0, 8).map(file => <span key={file.path} className="rounded-md bg-white/5 px-2 py-1 font-mono text-[10px] text-muted-foreground">{file.path}</span>)}</div></div> : null}{item.inspection?.kind === "archive" && item.artifact ? <div className="mt-3 rounded-lg border border-primary/15 bg-primary/[0.03] p-2.5"><div className="flex flex-col gap-2 sm:flex-row"><Input value={archiveQueries[item.id] ?? ""} onChange={event => setArchiveQueries(current => ({ ...current, [item.id]: event.target.value }))} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); runArchiveSearch(item); } }} className="h-9 text-xs" placeholder="ابحث في أسماء الملفات أو محتوى الشيفرة…" /><Button type="button" size="sm" onClick={() => runArchiveSearch(item)} disabled={searchArchive.isPending || (archiveQueries[item.id]?.trim().length ?? 0) < 2} className="h-9 bg-primary text-primary-foreground"><Search className="ml-1 h-3.5 w-3.5" />بحث</Button></div><p className="mt-2 text-[10px] leading-4 text-muted-foreground">يفحص Harb الملفات النصية الآمنة فقط؛ يعرض حتى 20 نتيجة ومقطعاً محدوداً لكل نتيجة.</p>{archiveResults[item.id] ? <div className="mt-2 space-y-1.5">{archiveResults[item.id]?.length ? archiveResults[item.id]?.map((result, index) => <div key={`${result.path}-${index}`} className="rounded-md border border-white/8 bg-black/15 px-2 py-1.5"><p dir="ltr" className="font-mono text-[10px] text-primary">{result.path}{result.line ? `:${result.line}` : ""}</p>{result.snippet && <p dir="ltr" className="mt-1 line-clamp-2 text-left text-[10px] leading-4 text-muted-foreground">{result.snippet}</p>}</div>) : <p className="rounded-md border border-dashed border-white/10 px-2 py-2 text-[11px] text-muted-foreground">لا توجد نتائج ضمن الملفات النصية الآمنة.</p>}</div> : null}</div> : null}{item.inspection?.textPreview ? <pre dir="ltr" className="mt-3 max-h-32 overflow-auto rounded-lg border border-white/8 bg-black/25 p-2 text-left text-[10px] leading-5 text-primary/90">{item.inspection.textPreview}</pre> : null}</article>)}</div>}
    </section>
  );
}
