...
el("check-execution-gate").addEventListener("click", async () => { try { const operation = el("operation-type").value; const checks = { run_program: window.harbAgent.checkProgramGate, run_command: window.harbAgent.checkCommandGate, modify_file: window.harbAgent.checkModifyFileGate }; const result = await checks[operation](); notice(result.reason, result.state === "awaiting_owner_approval" || result.state === "blocked" ? "error" : "success"); } catch (error) { notice(error.message, "error"); } });
